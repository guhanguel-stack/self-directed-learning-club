...
public class MyBookService {
    ...
    // ── 책 업로드 ─────────────────────────────────────────
    @Transactional
    public MyBookResponse uploadBook(Long userId,
                                     String title,
                                     String description,
        ...
        if (imageFile != null && !imageFile.isEmpty()) {
            imageKey = s3Service.uploadFile(imageFile, "my-books/images");
        }
        if (pdfFile != null && !pdfFile.isEmpty()) {
            fileKey = s3Service.uploadFile(pdfFile, "my-books/pdfs");
        }
        ...
        MyBook book = MyBook.builder()
                ...
                .fileUrl(fileKey)
                .owner(owner)
                .purchasedFromSite(false) // 직접 업로드 = 구매 아님
                .build();
