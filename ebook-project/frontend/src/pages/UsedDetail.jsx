...
import { getUsedListing, purchaseUsed, requestExchange, cancelUsedListing } from '../api/used';
...
const UsedDetail = () => {
  ...
  useEffect(() => {
    ...
    if (isLoggedIn) {
      getMyLibrary().then((res) => setMyLibrary(res.data.data.filter((b) => b.isAvailable)));
    }
  }, [listingId]);
  ...
  const handleExchange = async () => {
    if (!isLoggedIn) { navigate('/login'); return; }
    if (!selectedBook) { alert('교환할 책을 선택해주세요.'); return; }
    try {
      await requestExchange(listingId, Number(selectedBook));
        ...
        {!isMine && listing.status === 'ACTIVE' && (
          <div className="mt-8 border-t border-gray-100 pt-6">
            {listing.priceType === 'POINT' ? (
              ...
              <div>
                <p className="font-medium mb-3">교환할 내 책 선택</p>
                {myLibrary.length === 0 ? (
                  <p className="text-sm text-gray-400">교환 가능한 책이 없습니다.</p>
                ) : (
                  <>
                    <select
                      value={selectedBook}
