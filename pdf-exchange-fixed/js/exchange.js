...
async function acceptRequest(requestId) {
  ...
  showLoading('교환 수락 중...');
  try {
    // 요청 정보 가져오기 (Firestore)
    const reqRes = await apiGet('exchange_requests', { limit: 500 });
    const req = (reqRes.data || []).find(r => r.id === requestId);
    ...
    // 4. 교환된 책을 양측 책장에서 삭제 (status: EXCHANGED)
    await apiPatch('books', req.requested_book_id, { status: 'EXCHANGED' });
    await apiPatch('books', req.offered_book_id,   { status: 'EXCHANGED' });
