...
async function loadExploreBooks() {
  ...
  try {
    ...
    _allBooks  = (res.data || []).filter(b => b.status === 'ACTIVE' || !b.status);
    renderExploreGrid();
  } catch {
...
async function renderBookDetail(book) {
  ...
  // ── 파일 접근 가능 여부 (Firebase Storage URL 우선, IndexedDB fallback) ──
  const localExists = !!(book.pdf_url) || await hasPdfBlob(book.id).catch(() => false);
  ...
  if (hasAccess) {
    if (localExists) {
      ...
      return `<button class="btn btn-success" disabled>
                <i class="fas fa-info-circle"></i> 파일을 불러올 수 없습니다
              </button>`;
    }
  }
  if (!isLoggedIn()) {
...
function buildDetailNote(isMine, hasAccess, localExists, book) {
  ...
  if (hasAccess && !localExists) {
    return `<div class="detail-note note-warning">
              <p><i class="fas fa-exclamation-triangle"></i> 교환은 완료됐지만 파일을 불러올 수 없습니다.</p>
            </div>`;
  }
  return '';
}
...
async function loadMyLibrary() {
  ...
  try {
    ...
    const res = await apiGet('books', { limit: 500 });
    _myBooks  = (res.data||[]).filter(b => b.owner_email === currentUser.email && (b.status === 'ACTIVE' || !b.status));
...
async function renderMyBookGrid() {
  ...
  // 파일 접근 가능 여부 (Firebase URL 우선, IndexedDB fallback)
  const localFlags = await Promise.all(
    _myBooks.map(async b => !!(b.pdf_url) || await hasPdfBlob(b.id).catch(() => false))
  );
  ...
  try {
    ...
    for (const bid of bookIds) {
      try {
        ...
        if (b) {
          // Firebase Storage URL 우선, IndexedDB fallback
          const localExists = !!(b.pdf_url) || await hasPdfBlob(bid).catch(() => false);
